...
static void _nrf_provc_db_disc_handler(ble_db_discovery_evt_t * disc_evt)
{
    if (   (disc_evt->evt_type != BLE_DB_DISCOVERY_COMPLETE)
        || (disc_evt->params.discovered_db.srv_uuid.uuid != MESH_PROV_SVC_UUID)
...
static void _nrf_provc_on_ble_gap_evt_connected(ble_gap_evt_t const * gap_evt)
{
    ...
    ble_db_discovery_init(_nrf_provc_db_disc_handler);
...
static void _nrf_provc_on_ble_gap_evt_disconnected(ble_gap_evt_t const * gap_evt)
{
}
...
static void _nrf_provc_on_ble_gattc_evt_write_rsp(ble_gattc_evt_t const * gattc_evt)
{
    if (g_nrf_provc_data_in_hdl == gattc_evt->params.write_rsp.handle)
    {
        ...
        nrf_ipc_write(NRF_IPC_TAG_PROVC_SENT, (nrf_ipc_write_param_t*)&sent);
    }
    else if (g_nrf_provc_data_out_cccd_hdl == gattc_evt->params.write_rsp.handle)
    {
        sd_ble_gattc_exchange_mtu_request(gattc_evt->conn_handle, MESH_PROV_SVC_MTU);
    }
}

static void _nrf_provc_on_ble_gattc_evt_hvx(ble_gattc_evt_t* gattc_evt)
{
    ...
    if (g_nrf_provc_data_out_hdl == hvx_evt->handle)
    {
        ...
        data.data = (uint8_t*)hvx_evt->data;
        data.data_len = hvx_evt->len;
        data.conn_handle = gattc_evt->conn_handle;
        nrf_ipc_write(NRF_IPC_TAG_PROVC_DATA_IN, (nrf_ipc_write_param_t*)&data);
    }
}

void nrf_provc_on_ble_evt(ble_evt_t* ble_evt)
{
    ...
    switch (ble_evt->header.evt_id)
    {
        ...
        case BLE_GATTC_EVT_HVX:
            _nrf_provc_on_ble_gattc_evt_hvx(&ble_evt->evt.gattc_evt);
            break;
