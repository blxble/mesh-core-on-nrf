...
static void _nrf_proxys_on_ble_gap_evt_connected(ble_gap_evt_t const * gap_evt)
{
...
void nrf_proxys_on_ble_evt(ble_evt_t* ble_evt)
{
    ...
    switch (ble_evt->header.evt_id)
    {
        case BLE_GAP_EVT_CONNECTED:
            ...
            _nrf_proxys_on_ble_gap_evt_connected(gap_evt);
            break;

        case BLE_GAP_EVT_DISCONNECTED:
            gap_evt = &ble_evt->evt.gap_evt;
            _nrf_proxys_on_ble_gap_evt_disconnected(gap_evt);
            break;
...
void smport_proxy_server_send_pdu(uint8_t* data, uint16_t len)
{
    ...
    ble_gatts_hvx_params_t hvx_param =
    {
        ...
        .handle = g_nrf_proxys_char_out_hdl.value_handle,
        .p_len  = &len,
    };
